...
public class SHRED {
		...
		*/
		...
		public int[] SP,FFID,RCV_NB,STREAMER_NO;
...
public static void main(String[] args) {
		...
		public SHRED() throws IOException, InterruptedException {
			initialize();
		}
		...
		//	OUTPUT=new float[number_of_chan_per_streamer*nos][];
			SP=new int[number_of_chan_per_streamer*nos];
			FFID=new int[number_of_chan_per_streamer*nos];
			RCV_NB=new int[number_of_chan_per_streamer*nos];
			STREAMER_NO=new int[number_of_chan_per_streamer*nos];
		...
		public void frame()
		{
			...
			F.setLayout(null);
			F.setTitle("SHRED SEQ: "+seq_no);
			Image img=new ImageIcon("ICONS//SHRED.png").getImage();
			F.setIconImage(img);
		...
		//	P9.setBorder(BorderFactory.createLineBorder(Color.BLACK));
			P24=new XLabel2();
			P8.add(P7);P8.add(P9);P8.add(P24);
			//Seismic Panel
			GLP1=new DrawPanel();
	...
	//		TP.repaint();
		...
		public void buttons()
		{
			...
			//	ImageIcon icon=new ImageIcon("ICONS//"+"r"+String.valueOf(i)+".png");
				B5[i]=new JButton();
				B5[i].setMargin(new Insets(0,0,0,0));
				...
				P15.add(B5[i]);
				  ...
				  B5[0].setText("<<");B5[1].setText("||");B5[2].setText(">>");
				  
				  B5[0].setToolTipText("Forward");
				  B5[1].setToolTipText("Pause");
				  B5[2].setToolTipText("Backward");
							...
							public void actionPerformed(ActionEvent e)
							{
								...
								if(e.getSource()==B7[4])
								{
									try {
										...
										textfile="PROJECTS/"+project_name+"/"+seq_no+"/shred.jpg";
										File f=new File(textfile);
										int k=1;
										while(f.exists())
										{
										textfile="PROJECTS/"+project_name+"/"+seq_no+"/shred_"+String.valueOf(k)+".jpg";
										f=new File(textfile);
										k=k+1;
										}
										try {
		...
		try {
			...
			B5[1].setBackground(Color.yellow);
			B5[0].setBackground(B1[1].getBackground());
			B5[2].setBackground(B1[1].getBackground());
		...
		public void stop() throws InterruptedException
		{
			...
			if(FFID[FFID.length-1]==0)
			shot=fsp;
			else
			shot=FFID[FFID.length-1]+1;
			F.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
		}
		...
		public void start()
		{
			...
			if(FFID[FFID.length-1]==0)
				...
				shot=FFID[FFID.length-1]+1;
			if(STREAMER_NO[STREAMER_NO.length-1]==0)
				streamer=1;
				else
				{
					streamer=STREAMER_NO[STREAMER_NO.length-1]+1;
					if(streamer>number_of_streamers)
						streamer=1;
				}
			...
			B5[2].setBackground(Color.green);
			B5[1].setBackground(B1[1].getBackground());
			B5[0].setBackground(B1[1].getBackground());
		...
		{
			shot=FFID[0]-1;
			streamer=STREAMER_NO[0]-1;
			if(streamer==0)
			streamer=number_of_streamers;
			goback g=new goback();
			exit=false;
			g.start();
			start=1;
		...
		public void resize()
		{
			...
			P24.setBounds(10, P9.getY(), P9.getX()-10, P9.getHeight());
			P24.setLayout(null);
			//Gun Pressure Panel
			//SP button location
			SPB.setBounds(P23.getX()+P23.getWidth()+10,P23.getY(),200,P23.getHeight());
		...
		JTextField t3,t4,t5;
		int error_control,cur_fill,cur_reverse,cur_grid,gg3;
		...
		public void settings() throws IOException
		{
			...
			DP.setLayout(null);
			dp1.setLayout(null);dp4.setLayout(null);
			...
			dp4.setBounds(10,170,350,50);dp4.setBorder(BorderFactory.createLineBorder(Color.BLACK));
			b1.setBounds(10, 230, 100, 30);
			b2.setBounds(120, 230, 150, 30);
			
			//Add Panels
			DP.add(dp1);DP.add(dp4);
			//Add Buttons
			DP.add(b1);DP.add(b2);
			//Panel1
			//Labels
		...
		static float timee;
	 	public class transfer extends Thread{
			...
			//	    T1.setText(String.valueOf(shot));
					 ...
					 SPB.setText("FFID: "+String.valueOf(shot)+" "+"SP:"+String.valueOf(SP[SP.length-1]));
					 SPB.setBackground(Color.green);
					 SPT.setText(String.valueOf(shot));
					 SPT.repaint();
	 	...
	 	public class goback extends Thread{
			 //Initially setting the flag as true
			 //This method will set flag as false
			 public void stopp() throws IOException
			   {
		...
		public void readsegd(String filename)
		{
			...
			/*
				...
				/*
				for(int m=0;m<number_of_streamers;m++)
				{
				for(int i=0;i<number_of_chan_per_streamer;i++)
				{
					pos=trace_start_point+trace_block_size*(i+aux+number_of_chan_per_streamer*m)+header_size;
					...
					fb.get(input); 
				 float[] fd=filter2(input);
				 SUM=new float[number_of_sample];
				 
				 float sum=0;
				 for(int j=0;j<number_of_sample;j++)
				 {
					SUM[j]=SUM[j]+fd[j];
				//	OUTPUT[kk][j]=OUTPUT[kk][j]+fd[j];
					ALL[m][kk][j]=(ALL[m][kk][j]+fd[j]);
				 }
				}
				}
				
			
				int start_shot=number_of_chan_per_streamer*(nos-1);
				for(int i=0;i<start_shot;i++)
				{
					OUTPUT[i]=OUTPUT[i+number_of_chan_per_streamer];
					SP[i]=SP[i+number_of_chan_per_streamer];
					FFID[i]=FFID[i+number_of_chan_per_streamer];
					RCV_NB[i]=RCV_NB[i+number_of_chan_per_streamer];
					STREAMER_NO[i]=STREAMER_NO[i+number_of_chan_per_streamer];
				}
				for(int i=0;i<number_of_chan_per_streamer;i++)
				{
					pos=trace_start_point+trace_block_size*(i+aux+number_of_chan_per_streamer*(streamer-1))+header_size;
					byte[] AMP = Arrays.copyOfRange(b, pos, pos+amp_size);
					FloatBuffer fb = ByteBuffer.wrap(AMP).asFloatBuffer();
					input = new float[number_of_sample];
					fb.get(input); 
					float[] fd=filter2(input);
					OUTPUT[start_shot+i]=fd;
					SP[start_shot+i]=sp_number;
					FFID[start_shot+i]=file_number;
					RCV_NB[start_shot+i]=i+1;
					STREAMER_NO[start_shot+i]=streamer;
				 }
		...
		public void amplitudes(byte[] b)
		{
			 ...
			 byte[] g1;
		      ...
		      if(OUTPUT==null)
		      {
		    	  OUTPUT=new float[number_of_chan_per_streamer*nos][number_of_sample];
		      }
		      ...
		      int pos_start=pos+trace_position;
		      
		      //VERININ SONUNA EKLEME  
		  	for(int i=0;i<number_of_chan_per_streamer;i++)
			{
				pos=pos_start+i*(trace_size+header_size)+header_size;
		...
		public void readsegdback(String filename)
		{
			byte[] b;
			byte[] g1;
			File f=new File(filename);
		...
		//		int size=b.length;
			...
			//	int number_of_chan=(size-trace_start_point)/(trace_block_size)-4;
				...
				float[][] OUTPUT1=new float[number_of_traces][];
				...
				*/
				...
				for(int i=0;i<start_shot;i++)
				{
					OUTPUT[l-i]=OUTPUT[l-(i+number_of_chan_per_streamer)];
					SP[l-i]=SP[l-(i+number_of_chan_per_streamer)];
					FFID[l-i]=FFID[l-(i+number_of_chan_per_streamer)];
					RCV_NB[l-i]=RCV_NB[l-(i+number_of_chan_per_streamer)];
					STREAMER_NO[l-i]=STREAMER_NO[l-(i+number_of_chan_per_streamer)];
				}
				for(int i=0;i<number_of_chan_per_streamer;i++)
				{
					pos=trace_start_point+trace_block_size*(i+aux+number_of_chan_per_streamer*(streamer-1))+header_size;
		        ...
		        addMouseMotionListener(new MouseMotionAdapter() {
		        	 ...
		        	 public void mouseMoved(MouseEvent e) {
		        		 if(OUTPUT!=null )
		        		 {
		        			 ...
		        			 L[0].setText("FFID: "+c);
		        			 if(a<SP.length && OUTPUT[a]!=null)
		        			 {
		        			 L[1].setText("SP NO: "+SP[a]);
			 ...
			 @Override
			    ...
			    */
			    ...
			    for(int j=0;j<SP.length;j++)
	    		{
	    			...
	    			ss=String.valueOf((SP[j]));
	    			String kk=String.valueOf((FFID[j]));
	    			String nn=String.valueOf((RCV_NB[j]));
	    			String bb=String.valueOf((STREAMER_NO[j]));
	    			int xx=(int)(((float)ss.length()/2f)*6f);
	    			g.drawString(ss,(int) (x-xx),super.getHeight()-15);
	    			g.drawString(kk,(int) (x-xx),super.getHeight()-35);
	    			g.drawString(nn,(int) (x-xx),super.getHeight()-55);
	    			g.drawString(bb,(int) (x-xx),super.getHeight()-75);
		...
		public class XLabel2 extends JPanel{
			 ...
			 @Override
			...
			
	    		{
	    			...
	    			String bb="CABLE_NO";
	    			int xx=(int)(((float)ss.length()/2f)*6f);
	    			g.drawString(ss,5,super.getHeight()-15);
	    			g.drawString(kk,5,super.getHeight()-35);
	    			g.drawString(nn,5,super.getHeight()-55);
	    			g.drawString(bb,5,super.getHeight()-75);
	    			//g.drawLine(x,super.getHeight()-10,x,super.getHeight());
