...
//import NFH.VERICellRenderer;
//import NFH.VERITableModel;
...
public class ALD {
	...
	public static JPanel[] P;		//Panels for General-Buttons-Statusr
	...
	/*
	public int number_of_string=2;
	public int number_of_chan=1;
	public int number_of_array=2;
	public static int fsp=950;
	public static int lsp=2475;
	public String project_name="TPGM19";
	public String seq_no="207";
	String filepath;
	*/
	 ...
	 */
	public ALD() throws IOException, InterruptedException {
		initialize();
	}
	...
	public class FileNames extends Thread{
	 ...
	 VERITableModel tm = new VERITableModel();
	 VERITableModel tm_ok = new VERITableModel();
	 VERITableModel tm_na = new VERITableModel();
	 VERITableModel tm_comm = new VERITableModel();
	...
	*/
		...
		//	tm.addColumn("Time");tm.addColumn("Action");tm.addColumn("Status");
			...
			LF.setTitle("ALD Log");
			Image img=new ImageIcon("ICONS//ALD.png").getImage();
			LF.setIconImage(img);
	...
	public void readfile()
	{
		...
		try {
			...
			number_of_string=number_of_array;
			number_of_chan=1;
	...
	public void settings() throws IOException
	{
		...
		DP.setBounds(F.getX()+100,F.getY()+ 100, 400,430);
		...
		dp3.setBounds(10,280,350,50);dp3.setBorder(BorderFactory.createLineBorder(Color.BLACK));
		b1.setBounds(10, 350, 100, 30);
		b2.setBounds(120, 350, 150, 30);
		...
		//Panel2 Labels
		JLabel l4=new JLabel("Trace Gap Between Array");JLabel l5=new JLabel("Sample Gap Between Cluster");
	...
	public void gap()
	{
		...
		{
			col=DATA[0].size()*(number_of_string/2)+DATA[1].size()*(number_of_string/2)+(number_of_string-1)*gapx;
		}
		...
		for(int i=0;i<DATA.length;i++)
		{
			...
			for(int j=0;j<DATA[i].size();j++)
			{
				...
				for(int k=0;k<18;k++)
				{
					if(scol+j<D[0].length && startrow+k<D.length)
					{
	...
	public void initialize() throws IOException, InterruptedException {
		...
		F.setTitle("AIR LEAK DETECTION - Sequence No: "+seq_no+" "+"Referance Sequence:"+ref_seq);
		Image img=new ImageIcon("ICONS//ALD.png").getImage();
		F.setIconImage(img);
		   ...
		   MP.addGLEventListener( new GLEventListener() {
	         ...
	         //       Testgjp.setup( glautodrawable.getGL().getGL2(), width, height );
	            ...
	            public void display( GLAutoDrawable glautodrawable ) {
	            	if(start==1 && DATA[0].size()>0 && COLS.size()>0)
	            	{
	            		 plot(glautodrawable );
	            	}
	            	else
		...
		LP.setLayout(null);
		TP.setLayout(null);
		P[0].add(P[1]);P[0].add(P[2]);P[0].add(MP);
		P[0].add(LP);
		P[0].add(TP);
		//Scroll Bars
		sx.setEnabled(false);
				...
				//Select Trace
				 B5=new JButton[2];
					...
					p9.add(B5[i]);
					 ...
					 B5[0].setToolTipText("Array down");
					 B5[1].setToolTipText("Array up");
						...
						public void actionPerformed(ActionEvent e)
						{
							...
							if(e.getSource()==B6[5])
							{
								try {
									...
									textfile="PROJECTS/"+project_name+"/"+seq_no+"/ald.jpg";
									File f=new File(textfile);
									int k=1;
									while(f.exists())
									{
									textfile="PROJECTS/"+project_name+"/"+seq_no+"/ald_"+String.valueOf(k)+".jpg";
									f=new File(textfile);
									k=k+1;
									}
									try {
	...
	public void savereport() throws IOException
	{
		...
		try {
			...
			raf.write(fsp);raf.write(lsp);
			raf.write(good_shot);raf.write(missing_shot);raf.write(comm_error);
	...
	{
		/*
		File mpf=new File("//users//geo4//Desktop//Java_Project//MTAQC//MUSIC//queen.wav");
		AudioInputStream ais=AudioSystem.getAudioInputStream(this.getClass().getResource("//users//geo4//Desktop//Java_Project//MTAQC//MUSIC//queen.wav"));
	...
	public void button_active(JButton[] B)
	{
		...
		B2[0].setEnabled(false);
		B2[1].setEnabled(false);
		B6[1].setEnabled(false);
		B6[2].setEnabled(false);
		B6[3].setEnabled(false);
	...
	public static void plot( GLAutoDrawable glautodrawable ) {
		...
		// min=min();
		 ...
		 if(density==1)
		 {
			 for(int i=0;i<x;i++)
			 {
				 renk=findcols(D[starty][i+startx]*scale);
				 if(D[starty][i+startx]!=9999)
				 ...
				 { 
					 ...
					 gl.glVertex2f(-1f+width*i,1f-height*(j));
					 gl.glVertex2f(-1f+width*(i+1),1f-height*(j));
					 gl.glVertex2f(-1f+width*(i),1f-height*(j+1)); 
					 gl.glVertex2f(-1f+width*(i+1),1f-height*(j+1));
					 }
				 }
				 gl.glEnd();
			 }
	 ...
	 public void disp_resize(Point ps,Point pe)
		{
		 ...
		 starty=startyy+(int)((float)((float)y1/((float)MP.getHeight())*(endyy-startyy)))+1;
		 endy=startyy+(int)((float)((float)y2/((float)MP.getHeight())*(endyy-startyy)))+1;
	...
	public void resize() throws IOException, InterruptedException
	{
		...
		MP.setBounds(100, 75+50, P[0].getWidth()-220, P[0].getHeight()-(225));
	...
	public  void read(String file) throws IOException
	{
		...
		if(noc_cal==18)
		{
			...
			float[] diff=new float[number_of_chan];
	...
	public float diff(float[] A,float[] B)
	{
		...
		float diff=0;
		//Diff between ref and real data
		for(int i=0;i<C.length;i++)
		{
			float a=((C[i]/max1)-D[i]/max2);
			diff=diff+a*a;
		}
		C=null;
		D=null;
	...
	public class MouseTest extends GLJPanel {
	    ...
	    public void paintComponent(Graphics g) {
 	       ...
 	       if(start==1)
		    {
		    	...
		    	if(grid==1)
		    	{
		    		int total_chan=endy-starty;
			    	for(int i=0;i<total_chan;i++)
			    	{
			    		float cyer=MP.getHeight()/total_chan;
			    		int y=(int)(cyer*(i+1));
			    		String ss=String.valueOf(starty+i+1);
	...
	public class Labels extends JPanel{
		 ...
		 @Override
		   ...
		   /*
		    //Time değerinin yazılması
		    if(!(pe==null))
	    	{
		    	int x=pe.x;
		    	int y=pe.y;
		    	float ch=(float)x/(float)p.getWidth();	
		    	ch=a1+ch*(a2-a1);
		    	ch=(ch+1)/2f;
		    	cc=(int)(ch*not);
		    float ti=(float)pe.y/(float)p.getHeight();
		    ti=a4-ti*(a4-a3);
	    	ti=(ti+1)/2f;
	    	int tt=nos-(int)(ti*nos);
	    	String cur_time="Zaman: "+String.valueOf(sam_int*tt)+" ms";
	    	String amp= "Genlik: "+String.format ("%.4f", DATA[tt][cc]);
	    	 g.drawString(cur_time,10 , 15); 	
	    	 g.drawString(amp,130 , 15);
	    	}
		    for(int i=0;i<selected_status.length;i++)
		    {
		    	if(!(pe==null))
		    	{
			    	String s=HEADERS[selected_status[i]]+" "+HDR[selected_status[i]][cc]+",";
			    	 g.drawString(s,(i+2)*120+10 , 15); 	
		    	}
