...
public class FFT {
    ...
    //Compute complex value to DB
    public static float[] FFT2AMP(float[] INPUT)
    {
    	int n=INPUT.length;
    	...
    	OUTPUT[i]=(float)Math.sqrt((double)OUTPUT[i]);
    	}
    	return OUTPUT;
    }
    
    public static float[] AMP2DB(float[] INPUT)
    {
    	int n=INPUT.length;
    	...
    	for(int i=0;i<n;i++)
    	{
    	OUTPUT[i]=(float) (20*Math.log((double)(INPUT[i]/a)));
    	}
    ...
    ***************************************************************************/
    ...
    public static float[] low(float[] input,float value,float sample_int)
    {
    	int sample=(input.length-1)/2;
    	float[] output=input;
    	float nyquist=(1f/(2*sample_int))*1000;
    	float starting_point=value/((nyquist*2)/sample);
    	int sp=(int)(starting_point*2);
    	float slope=1f/starting_point;
    	for(int i=0;i<sp;i++)
    	{
    		output[i]=0;
    	}
    	
    	for(int i=input.length-sp;i<input.length;i++)
    	{
    		output[i]=0;
    	}
    	return output;
    ...
    public static float[] four1(float[] data1, int nn, int isign)
    {
        ...
        for (i = 1; i < n; i += 2)
        {
            if (j > i)
            {
                tempr = data1[j]; data1[j] = data1[i]; data1[i] = tempr;
                tempr = data1[j + 1]; data1[j + 1] = data1[i + 1]; data1[i + 1] = tempr;
            }
            m = n >> 1;
            while (m >= 2 && j > m)
            {
        ...
        {
            istep = 2 * mmax;
            theta = (float)((Math.PI) * 2) / (isign * mmax);
